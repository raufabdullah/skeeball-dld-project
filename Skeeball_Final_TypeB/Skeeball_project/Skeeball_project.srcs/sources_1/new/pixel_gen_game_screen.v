...
//    // Modified to include only red, blue, magenta, and transitions (no yellow, green, cyan)
//    if (hue < 8'd85) begin
//        // Red (F,0,0) to Blue (0,0,F)
//        digit_red <= 4'hF - ((hue * 6) / 16); // Scale 0-84 to F-0
//        digit_green <= 4'h0;
//        digit_blue <= (hue * 6) / 16; // Scale 0-84 to 0-F
//    end else if (hue < 8'd170) begin
//        // Blue (0,0,F) to Magenta (F,0,F)
//        digit_red <= ((hue - 8'd85) * 6) / 16; // Scale 0-84 to 0-F
//        digit_green <= 4'h0;
//        digit_blue <= 4'hF;
//    end else begin
...
//        digit_red <= 4'hF;
//        digit_green <= 4'h0;
//        digit_blue <= 4'hF - (((hue - 8'd170) * 6) / 16); // Scale 0-85 to F-0
//    end
//end
...
always @(posedge clk_d) begin
    ...
    // RGB spectrum transition: red -> magenta -> blue -> red
    if (hue < 8'd85) begin
        // Red (F,0,0) to Magenta (F,0,F)
        digit_red <= 4'hF;
        digit_green <= 4'h0;
        digit_blue <= (hue * 6) / 32; // Scale 0-84 to 0-F
    end else if (hue < 8'd170) begin
        // Magenta (F,0,F) to Blue (0,0,F)
        digit_red <= 4'hF - (((hue - 8'd85) * 6) / 32);
        digit_green <= 4'h0;
        digit_blue <= 4'hF;
    end else begin
        // Blue (0,0,F) to Red (F,0,0)
        digit_red <= ((hue - 8'd170) * 6) / 34; // Scale 170-255 to 0-F
        digit_green <= 4'h0;
        digit_blue <= 4'hF - (((hue - 8'd170) * 6) / 34);
    end
end
...
always @(posedge clk_d) begin
    if (video_on) begin
        ...
        else if (pixel_x >= 212 && pixel_x <= 428 && pixel_y >= 10 && pixel_y <= 100 ||
                 pixel_x >= 205 && pixel_x <= 435 && pixel_y >= 17 && pixel_y <= 93) begin
