...
module Top_Level_Module(
    ...
    // RGB outputs from start and end screen pixel generators
    ...
    // Select RGB outputs based on begin input
    assign red = (start == 0) ? red_start : red_end;
    assign green = (start == 0) ? green_start : green_end;
    assign blue = (start == 0) ? blue_start : blue_end;
    ...
    /// v_counter vc(trig_v, clk_d, v_count); 
    /// vga_sync vgas(h_count, v_count, h_sync, v_sync, video_on, x_loc, y_loc); 
    /// pixel_gen_endscreen pg_es(clk_d, x_loc, y_loc, video_on, red, green, blue);
